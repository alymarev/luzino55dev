...
if ($_FILES['upload']) {
    if (($_FILES['upload'] == "none") OR (empty($_FILES['upload']['name']))) {
        ...
        move_uploaded_file($_FILES['upload']['tmp_name'], $_SERVER["DOCUMENT_ROOT"]."/images/articles/" . $name);
        $full_path = Config::SITE_ADDRESS . '/images/articles/' . $name;
        $message = "Файл " . $_FILES['upload']['name'] . " загружен";
        $size = @getimagesize('images/' . $name);
//        if($size[0]<50 OR $size[1]<50){
//            unlink('/images/articles/'.$name);
