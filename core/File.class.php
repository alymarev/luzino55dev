...
class File {
    ...
    public static function uploadDocs($file,$max_size, $dir, $root = false, $source_name = false){
        $blacklist = array(".php", ".phtml", ".php3", ".php4", ".html", ".htm");
        foreach ($blacklist as $item)
            if (preg_match("/$item\$/i", $file["name"])) throw new Exception("ERROR_DOCS_TYPE");
        $type = $file["type"];
        $size = $file["size"];
        if ($size > $max_size) throw new Exception("ERROR_AVATAR_SIZE");
        if ($source_name) $avatar_name = $file["name"];
        else $avatar_name = self::getName().".".substr($file["name"],strpos($file["name"],".")+1,strlen($file["name"]));
        $upload_file = $dir . $avatar_name;
        if (!$root) $upload_file = $_SERVER["DOCUMENT_ROOT"] . $upload_file;
        if (!move_uploaded_file($file["tmp_name"], $upload_file)) throw new Exception("UNKNOWN_ERROR");
        return $avatar_name;
