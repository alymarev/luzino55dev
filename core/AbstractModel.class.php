...
abstract class AbstractModel {
    ...
    protected static function getAllOnWhere($table_name, $class, $where = false, $values = false, $order = false, $ask = true, $count = false, $offset = false)
    {
        ...
        return AbstractModel::buildMultiple($class, $data);
    }
    ...
    public static function buildMultiple($class, $data) {
        ...
        $test_obj = new $class();
        if (!$test_obj instanceof AbstractModel) throw new Exception();
        foreach ($data as $row) {
            $obj = new $class();
            $obj->init($row);
            $ret[$obj->getID()] = $obj;
    ...
    protected static function searchObjects($select, $class, $fields, $words, $min_len) {
        ...
        $results = Model::buildMultiple($class, $results);
        ...
        uasort($data, array("AbstractModel", "compareRelevant"));
        return $data;
    }
